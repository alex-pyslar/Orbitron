...
```orbitron
var (a, b) = (1, 2);        // деструктурирующее присвоение
var (x, y) = (y, x);        // обмен значений
var point  = (10, 20);      // кортеж как значение
```
...
```orbitron
@test
func test_something() { assert_eq(1 + 1, 2); }

@deprecated
func old_api(x: int): int { return x; }

@override
pub func speak(self) { println("Гав!"); }

@inline
func fast_mul(a: int, b: int): int { return a * b; }
```
...
var double = |x| x * 2;
var add    = |a, b| a + b;
var result = |x| { var y = x * 2; return y + 1; };
...
impl Point {
    ...
    static func origin() -> int { return 0; }   // статический метод
}

var p = Point { x: 3, y: 4 };   // без new!
println(p.len_sq());              // 25
p.move_by(1, 0);
var o = Point::origin();         // статический вызов
```
...
class Counter {
    ...
    static func zero(): int { return 0; }   // статический метод
}

var c = new Counter(0, 5);
c.tick();
println(c.get());           // 5
var z = Counter::zero();    // статический вызов
```
...
impl Measurable for Circle {
    func area(self): int { return 314 * self.radius * self.radius / 100; }
    func perimeter(self): int { return 628 * self.radius / 100; }
}

// Реализация операторного трейта
impl Add for Circle {
    func add(self, other_r: int): int {
        return self.radius + other_r;
    }
}
```
...
program    = ( func_decl | struct_decl | impl_decl | class_decl
             ...
             | trait_decl | impl_for_decl )* ;

import_decl    = 'import' STRING ';' ;
const_decl     = 'const' IDENT ':' type '=' expr ';' ;
enum_decl      = 'enum'  IDENT '{' (IDENT ',')* '}' ;
trait_decl     = 'trait' IDENT '{' trait_method* '}' ;
trait_method   = ['static'] 'func' IDENT '(' param_list ')' [(':' | '->') type] ';' ;
impl_for_decl  = 'impl' IDENT 'for' IDENT '{' method_decl* '}' ;
func_decl      = ['@' IDENT] ['static'] 'func' IDENT '(' param_list ')'
                 [(':' | '->') type] block ;
class_decl     = ['class' IDENT ['extends' IDENT] '{' field_list method_list '}'] ;

block = '{' stmt* '}' ;
stmt  = var_stmt | tuple_assign | assign | compound_assign | if_stmt | unless_stmt
...
tuple_assign   = 'var' '(' IDENT (',' IDENT)* ')' '=' '(' expr_list ')' ';' ;

for_stmt = 'for' IDENT 'in' (range_expr | IDENT) (',' IDENT 'in' range_expr)* block ;

expr      = pipe_expr ;
pipe_expr = ternary ('|>' (IDENT | lambda) ['(' arg_list ')'])* ;
